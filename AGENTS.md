...
- 1s replay segments with a complete emulator restart between each SRAM save and the following load, repeated until every available US replay finishes
...
Run the whole US suite with `just test-save-state-replay-suite`. Run one step with `just test-save-state-replay-suite-step STEP`, where `STEP` is `regular`, `default`, `long`, `cameras`, `near-end`, or `cold-restart`. Run every applicable step for one replay with `just test-save-state-replay-suite-replay REPLAY`, or combine both selectors with `just test-save-state-replay-suite-step STEP REPLAY`. Replay names may include or omit the `.ram` suffix (for example, `01-dam` or `01-dam.ram`).
