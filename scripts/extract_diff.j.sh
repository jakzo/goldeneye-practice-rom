    ...
    do
        if [ "$extract" == "$true" ]; then
            if [ "$compressed" == "$true" ]; then
                ...
                dd if=baserom.j.z64 of="${name}.temp" iflag=skip_bytes,count_bytes skip="$offset" count="$size" status=none
    	        GZ=gzip tools/1172inflate.sh "${name}.temp" "${name}"
                rm "${name}.temp"
            else
                echo "Extracting uncompressed $name, $size bytes..."
                dd if=baserom.j.z64 of="$name" iflag=skip_bytes,count_bytes skip="$offset" count="$size" status=none
