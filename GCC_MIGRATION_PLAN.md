...
Every row uses the US `FINAL=YES` instrumented build and the deterministic
689-frame `REPLAY_RUNWAY_1X` capture in ares v148. "Total code size" is the
sum of the linked `.code`, `.inflate`, and `.game` sections; frame cycles are
`tick_cycles`; TLB loads are software code-page loads. This fixes the workload,
instrumentation, compiler policy, and emulator version so rows remain comparable.
...
Both baselines were captured on 2026-07-12. The base-decomp row was captured
from `base-decomp-replay-profile` at commit `d6c96a2c`; the Phase 0 row was
captured before any original translation unit was added to the mixed GCC
manifest. Machine-readable summaries and raw CSV files are generated by
`scripts/migration/profile_summary.py` from each instrumented ELF and profiler
capture.
