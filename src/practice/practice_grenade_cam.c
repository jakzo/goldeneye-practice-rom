...
extern void sub_GAME_7F0876C4(coord3d* cam_pos, coord3d* cam_look, coord3d* cam_up);
...
void practice_grenade_cam_tick(void) {
    ...
    if (g_CurrentPlayer == NULL || player_prop == NULL) return;
    ...
    for (s = 0; s < MAX_PIP_SLOTS; s++) {
        ...
        if (slot->freeze_ticks > 0) {
            ...
            still_active =
                p != NULL &&
                !(p->flags & PROJECTILEFLAG_FREE) &&
                p->obj == slot->grenade_obj &&
                p->obj->prop != NULL;
    ...
    for (i = 0; i < PROJECTILES_ARR_MAX; i++) {
        ...
        if (!is_player_grenade) continue;

        // Check if already tracked
        for (s = 0; s < MAX_PIP_SLOTS; s++) {
            if (g_PipSlots[s].active && g_PipSlots[s].grenade_obj == p->obj) goto next_grenade;
        }
...
Gfx *practice_grenade_cam_render(Gfx *gdl) {
    ...
    for (s = 0; s < MAX_PIP_SLOTS; s++) {
        ...
        gDPSetScissor(gdl++, G_SC_NON_INTERLACE, left, top, left + PIP_SIZE, top + PIP_SIZE);
        ...
        // Load a dynamically allocated viewport in g_VtxBuffers (flushed to physical RAM)
        // because the player struct is in stage BSS/heap and CPU modifications to it
        // are not coherent with the RSP's physical memory reads.
        {
            Vp *vp = (Vp*)dynAllocate(sizeof(Vp));
            vp->vp.vscale[0] = (g_ViBackData->viewx * 2);
            vp->vp.vtrans[0] = (g_ViBackData->viewx * 2) + (g_ViBackData->viewleft * 4);
            vp->vp.vscale[1] = (g_ViBackData->viewy * 2);
            vp->vp.vtrans[1] = (g_ViBackData->viewy * 2) + (g_ViBackData->viewtop * 4);
        ...
        if (stan == NULL) stan = saved_room_pointer;
        ...
        gDPFillRectangle(gdl++, left, top + PIP_SIZE - 1, left + PIP_SIZE - 1, top + PIP_SIZE - 1); // Bottom
        gDPFillRectangle(gdl++, left, top, left, top + PIP_SIZE - 1); // Left
        gDPFillRectangle(gdl++, left + PIP_SIZE - 1, top, left + PIP_SIZE - 1, top + PIP_SIZE - 1); // Right
        gDPPipeSync(gdl++);
    }
    ...
    *g_ViBackData = saved_vi;
    ...
    currentPlayerSetPerspective(saved_perspnear, saved_perspfovy, saved_perspaspect);
    currentPlayerSetCameraScale();
