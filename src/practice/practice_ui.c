...
extern s32 _Printf(char *(*pfn)(char *, const char *, size_t), char *, const char *, va_list);
...
static void ensure_timing_initialized(void) {
    if (g_LogLifetimeCycles != 0) return;
...
static int practice_vsnprintf(char *dst, const char *fmt, va_list args) {
    ...
    buf.dest = dst;
    buf.remaining = LOG_MESSAGE_MAX_LEN - 2; // reserve 2 for newline and null terminator
...
static void add_to_log_queue(LogLevel level, const char *fmt, va_list args) {
    ...
    // Ensure string ends with a newline so textMeasure calculates height correctly
    msg->text[len] = '\n';
    msg->text[len + 1] = '\0';
...
void practiceLogDebug(const char *fmt, ...) {
    va_list args;
    if (!practice.show_debug_logs) return;
    va_start(args, fmt);
...
Gfx *practice_ui_render(Gfx *gdl) {
    ...
    if (g_LogQueueCount == 0) return gdl;

    ensure_timing_initialized();

    gdl = microcode_constructor(gdl);
    
    // Reset the scissor box to cover the entire screen so we are not clipped by viewports
    ...
    for (i = g_LogQueueCount - 1; i >= 0; i--) {
        ...
        if (msg->width < 0) {
            textMeasure(&msg->height, &msg->width, msg->text, (struct fontchar *)LOGGER_FONT_CHARS, (struct font *)LOGGER_FONT_TABLE, 0);
        }
        ...
        gdl = textRenderGlow(gdl, &view_left, &view_vert, msg->text, LOGGER_FONT_CHARS, LOGGER_FONT_TABLE, (s32)color_fg, color_glow, (s16)(s32)viGetX(), (s16)viGetY(), 0, 0);
