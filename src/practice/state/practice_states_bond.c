...
void save_bond_state(SavedBondState *dst) {
  ...
  dst->field_488_current_tile_ptr_offset =
      get_tile_offset(g_CurrentPlayer->field_488.current_tile_ptr);
  dst->field_488_current_tile_ptr_for_portals_offset =
      get_tile_offset(g_CurrentPlayer->field_488.current_tile_ptr_for_portals);
  dst->previous_collision_info_current_tile_ptr_offset = get_tile_offset(
      g_CurrentPlayer->previous_collision_info.current_tile_ptr);
  dst->previous_collision_info_current_tile_ptr_for_portals_offset =
  ...
  dst->autoaim_target_y_index =
      get_prop_index(g_CurrentPlayer->autoaim_target_y);
  dst->autoaim_target_x_index =
...
void load_bond_state(SavedBondState *src) {
  ...
  // 1. Backup the hand weapon items and text overrides held before loading
  // state
  ...
  // 3. Resolve tile pointers and target prop pointers from saved
  // offsets/indices
  g_CurrentPlayer->room_pointer = get_tile_by_offset(src->room_pointer_offset);
  g_CurrentPlayer->field_488.current_tile_ptr =
      get_tile_by_offset(src->field_488_current_tile_ptr_offset);
  g_CurrentPlayer->field_488.current_tile_ptr_for_portals =
      get_tile_by_offset(src->field_488_current_tile_ptr_for_portals_offset);
  g_CurrentPlayer->previous_collision_info.current_tile_ptr =
      get_tile_by_offset(src->previous_collision_info_current_tile_ptr_offset);
  g_CurrentPlayer->previous_collision_info.current_tile_ptr_for_portals =
      get_tile_by_offset(
          src->previous_collision_info_current_tile_ptr_for_portals_offset);
  g_CurrentPlayer->field_2A70 = get_tile_by_offset(src->field_2A70_offset);

  g_CurrentPlayer->autoaim_target_y =
      get_enabled_prop_by_index(src->autoaim_target_y_index);
  g_CurrentPlayer->autoaim_target_x =
      get_enabled_prop_by_index(src->autoaim_target_x_index);

  // 4. Recalculate derived look parameters from restored vv_theta / vv_verta
  bondviewApplyVertaTheta();

  // 5. Sync player's world PropRecord properties (rather than overwriting
  // pointer)
  if (src->has_prop) {
    if (g_CurrentPlayer->prop != NULL) {
  ...
  // 8. Restore saved inventory items
  {
    ...
    for (i = 0; i < src->num_inv_items; i++) {
      ...
      if (type == INV_ITEM_WEAPON) {
        ...
        bondinvAddDoublesInvItem(src->inv_items[i].weapon_right,
                                 src->inv_items[i].weapon_left);
