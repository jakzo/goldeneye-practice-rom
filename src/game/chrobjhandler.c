...
/* Monitor renderer imported from goldeneye_optimized. */
#if defined(VERSION_JP) || defined(VERSION_EU)
#define MONITOR_GAMEPLAY_TIMER_DELTA g_JP_GlobalTimerDelta
#else
#define MONITOR_GAMEPLAY_TIMER_DELTA g_GlobalTimerDelta
#endif

#define MONITOR_IMAGE_INDEX_LIMIT 100
#define MONITOR_SURFACE_MODE_TRANSLUCENT 2
#define MONITOR_OPAQUE_ALPHA 255
#define MONITOR_ROTATION_SCALE 1.4141999f

extern u8 *g_GfxMemPos;

/* These constants were part of the original assembly rodata and are also
 * referenced by the following object deformation routine. */
const f32 D_80052CB4 = 0.85f;
const f32 D_80052CB8 = 0.15f;
const f32 D_80052CBC = 0.15f;
const f32 D_80052CC0 = 0.9f;

struct tvcmd {
  u32 type;
  s32 time;
  /** Transition duration, or random threshold for TVCMD_RANDSETCMDLIST. */
  u32 durationOrThreshold;
};

/** Advances a monitor animation and emits its textured display list. */
Gfx *process_monitor_animation_microcode(Model *model, ModelNode *node, MonitorRecord *screen, Gfx *gdl, s32 transformIndex, s32 surfaceMode)
{
    f32 monitor_render_delta = MONITOR_GAMEPLAY_TIMER_DELTA;

#ifdef PRACTICE_ROM
    /* Zero-duration transitions use an infinite increment. A paused delta of
     * zero would produce infinity times zero and raise an FP exception. Only
     * advance this render-only monitor state, which is restored after render. */
    if (g_IsRenderOnly && monitor_render_delta == 0.0f)
    {
        monitor_render_delta = 1.0f;
    }
#endif
    if (node && (node->Opcode & 0xff) == MODELNODE_OPCODE_DLCOLLISION)
    {
        Vertex *vertices = (Vertex *)g_GfxMemPos;
        g_GfxMemPos += 4 * sizeof(Vtx);
        Gfx *branchCommand = gdl++;
        union ModelRoData *nodeRoData = node->Data;
        union ModelRwData *nodeRwData = modelGetNodeRwData(model, node);
        sImageTableEntry *textureConfig;
        bool shouldYield = FALSE;

        // Process animation commands until the monitor yields for this frame.
        while (!shouldYield)
        {
            struct tvcmd *command = (struct tvcmd *) &screen->cmdlist[screen->offset];
            ...
            case TVCMD_SCROLLRELX:
                ...
                screen->xmidinc = 1.0f / command->durationOrThreshold;
                screen->xmidold = screen->xmid;
                screen->xmidnew = screen->xmid + command->time * (1.0f / 1024.0f);
                screen->offset += 3;
                break;
            case TVCMD_SCROLLRELY:
                screen->ymidfrac = 0.0f;
                screen->ymidinc = 1.0f / command->durationOrThreshold;
                screen->ymidold = screen->ymid;
                screen->ymidnew = screen->ymid + command->time * (1.0f / 1024.0f);
                screen->offset += 3;
                break;
            case TVCMD_SCROLLABSX:
                ...
                screen->xmidinc = 1.0f / command->durationOrThreshold;
                screen->xmidold = screen->xmid;
                screen->xmidnew = command->time * (1.0f / 1024.0f);
                screen->offset += 3;
                break;
            case TVCMD_SCROLLABSY:
                ...
                screen->ymidinc = 1.0f / command->durationOrThreshold;
                screen->ymidold = screen->ymid;
                screen->ymidnew = command->time * (1.0f / 1024.0f);
                screen->offset += 3;
                break;
            case TVCMD_SCALEABSX:
                screen->xscalefrac = 0.0f;
                screen->xscaleinc = 1.0f / command->durationOrThreshold;
                screen->xscaleold = screen->xscale;
                screen->xscalenew = command->time * (1.0f / 1024.0f);
                screen->offset += 3;
                break;
            case TVCMD_SCALEABSY:
                screen->yscalefrac = 0.0f;
                screen->yscaleinc = 1.0f / command->durationOrThreshold;
                screen->yscaleold = screen->yscale;
                screen->yscalenew = command->time * (1.0f / 1024.0f);
                screen->offset += 3;
                break;
            case TVCMD_SETTEXTURE:
                save_img_index_to_obj_ani_slot(screen, (void *)command->time);
                screen->offset += 2;
                break;
            case TVCMD_PAUSE:
                if (screen->pause60 >= 0)
                ...
                else
                {
                    ...
                    shouldYield = TRUE;
                    screen->pause60 = command->time;
                }
                break;
            case TVCMD_SETCMDLIST:
                save_ptr_monitor_ani_code_to_obj_ani_slot(screen, (u32 *) command->time);
                break;
            case TVCMD_RANDSETCMDLIST:
                if ((randomGetNext() >> 16) < command->durationOrThreshold)
                {
                    save_ptr_monitor_ani_code_to_obj_ani_slot(screen, (u32 *) command->time);
                }
                else
            ...
            case TVCMD_SETCOLOUR:
                screen->colfrac = 0.0f;
                screen->colinc = 1.0f / command->durationOrThreshold;

                screen->redold = screen->red;
                screen->rednew = ((u32)command->time >> 24) & 0xff;

                screen->greenold = screen->green;
                screen->greennew = ((u32)command->time >> 16) & 0xff;

                screen->blueold = screen->blue;
                screen->bluenew = ((u32)command->time >> 8) & 0xff;

                screen->alphaold = screen->alpha;
                screen->alphanew = command->time & 0xff;

                screen->offset += 3;
                break;
            case TVCMD_ROTATEABS:
                screen->rot = command->time * M_TAU_F / M_U16_MAX_VALUE_F;
                screen->offset += 2;
                break;
            case TVCMD_ROTATEREL:
                screen->rot += monitor_render_delta * command->time * M_TAU_F / M_U16_MAX_VALUE_F;
        ...
        // Advance scale, scroll, and colour transitions.
        if (screen->xscaleinc > 0.0f)
        {
            screen->xscalefrac += screen->xscaleinc * monitor_render_delta;
        ...
        {
            screen->yscalefrac += screen->yscaleinc * monitor_render_delta;
        ...
        {
            screen->xmidfrac += screen->xmidinc * monitor_render_delta;
        ...
        {
            screen->ymidfrac += screen->ymidinc * monitor_render_delta;
        ...
        {
            screen->colfrac += screen->colinc * monitor_render_delta;
        ...
        // Copy the collision quad and set up its render state.
        nodeRwData->DisplayListCollisions.gdl = gdl;
        nodeRwData->DisplayListCollisions.Vertices = vertices;

        vertices[0] = nodeRoData->DisplayListCollisions.Vertices[0];
        vertices[1] = nodeRoData->DisplayListCollisions.Vertices[1];
        vertices[2] = nodeRoData->DisplayListCollisions.Vertices[2];
        vertices[3] = nodeRoData->DisplayListCollisions.Vertices[3];

        if ((u32)screen->tconfig < MONITOR_IMAGE_INDEX_LIMIT)
        {
            textureConfig = &monitorimages[(s32)screen->tconfig];
        }
        else
        {
            textureConfig = screen->tconfig;
        }

        if (textureConfig != NULL)
        {
            f32 xPositiveFraction;
            f32 yPositiveFraction;
            f32 xNegativeFraction;
            f32 yNegativeFraction;
            f32 rotationCosine;
            f32 rotationSine;
            f32 rotationScale;

            xPositiveFraction = screen->xscale / 2.0f;
            yPositiveFraction = screen->yscale / 2.0f;
            xNegativeFraction = xPositiveFraction;
            yNegativeFraction = yPositiveFraction;

            if (screen->rot != 0.0f)
            {
                rotationCosine = cosf(screen->rot) * MONITOR_ROTATION_SCALE;
                rotationScale = MONITOR_ROTATION_SCALE;
                rotationSine = sinf(screen->rot);

                xPositiveFraction *= rotationCosine;
                rotationSine *= rotationScale;
                yPositiveFraction *= rotationSine;
                xNegativeFraction *= rotationSine;
                yNegativeFraction *= rotationCosine;
            }

            vertices[0].s = textureConfig->width  * (screen->xmid + xPositiveFraction) * 32.0f;
            vertices[0].t = textureConfig->height * (screen->ymid + yPositiveFraction) * 32.0f;

            vertices[1].s = textureConfig->width  * (screen->xmid - xNegativeFraction) * 32.0f;
            vertices[1].t = textureConfig->height * (screen->ymid + yNegativeFraction) * 32.0f;

            vertices[2].s = textureConfig->width  * (screen->xmid - xPositiveFraction) * 32.0f;
            vertices[2].t = textureConfig->height * (screen->ymid - yPositiveFraction) * 32.0f;

            vertices[3].s = textureConfig->width  * (screen->xmid + xNegativeFraction) * 32.0f;
            vertices[3].t = textureConfig->height * (screen->ymid - yNegativeFraction) * 32.0f;
        }

        // Apply the current colour uniformly to all four vertices.
        {
            u8 red = screen->red;
            u8 green = screen->green;
            u8 blue = screen->blue;
            u8 alpha = screen->alpha;
        ...
        if (screen->alpha < MONITOR_OPAQUE_ALPHA)
        {
            surfaceMode = MONITOR_SURFACE_MODE_TRANSLUCENT;
        }

        // Render the image
        gSPSetGeometryMode(gdl++, G_CULL_BACK);

        texSelect(&gdl, textureConfig, surfaceMode, transformIndex, 2);
        ...
        gSPBranchList(branchCommand++, gdl);
    }
